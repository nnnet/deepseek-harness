...
- `CONTEXT_WINDOW_EXCEEDED_CODE`：当请求超过模型上下文窗口时，无论通过 HTTP 异常抛出还是带内 finish 交付，两个 DeepSeek 适配器都使用的提供方无关 code。`isContextWindowExceededError(detail)` 是每个适配器对提供方详细信息应用的共享保守分类器，覆盖合并 code 形式（`context_length_exceeded`）、模型容量措辞，以及 llama.cpp 系服务器在 HTTP 500 信封中发出的被动措辞 `Context size has been exceeded`。
