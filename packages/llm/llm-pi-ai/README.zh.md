...
- **`requestImageMediaTypes` 是对端点的声明，而不是对它的检查**：没有任何机制可以询问网关它接受哪些格式，因此声明了端点会拒绝的编码时，只能由提供方在轮次中途拒绝，此时消息已经持久化。默认值保留 WebP，本适配器所面向的托管端点都能解码它；llama.cpp 衍生的本地路由需要自行缩小该列表。
