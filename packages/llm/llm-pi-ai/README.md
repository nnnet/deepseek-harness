...
- **`requestImageMediaTypes` is a claim about the endpoint, not a check of it** — nothing interrogates a gateway for the formats it accepts, so a route claiming an encoding its endpoint refuses is refused by the provider instead, mid-turn, after the message is durable. The default keeps WebP, which every hosted endpoint this adapter targets decodes; a local llama.cpp-derived route must narrow the list itself.
