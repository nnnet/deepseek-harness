...
- **Overflow recovery** — provider-confirmed overflow needs no capacity metadata: it bypasses normal pressure and retention, prunes, then attempts one balanced head reduction while leaving the newest indivisible unit. The head it replays is bounded by the window the failing request exceeded, read from durable `request/context` metadata, once the summary's `maxTokens`, the replayed system prompt and tools, and the compaction instruction are reserved; the recent tail that does not fit stays verbatim on the surface. A configured summarizer answers on a window that metadata does not describe, and a route advertising no window leaves nothing to derive from, so both replay the whole compactable head; a `maxTokens` that alone exhausts the window warns with the numbers. Retry is authorized whenever `surface.replaceGeneration` advances, including when pruning lands before later summary work throws. No replacement, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
...
| `maxTokens` | no (default `8192`) | Provider generation cap for the summarization call; may include reasoning tokens, and is reserved out of the overflow replay budget. |
...
- **The overflow replay budget covers only the conversation's own advertised window** — a configured separate summarizer, or a route that advertises no `contextWindow`, replays the whole compactable head, so its summarization call can still exceed a window this package cannot see.
