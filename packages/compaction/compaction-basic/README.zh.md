...
- **溢出恢复**：提供方已确认的溢出不需容量元数据。它会绕过常规压力与保留，执行剪枝，再尝试一次平衡头部缩减，并留下最新不可分单元。它重放的头部受失败请求所超出的那个窗口约束：该窗口来自持久化的 `request/context` 元数据，并在预留摘要的 `maxTokens`、重放的系统提示与工具，以及压缩指令之后确定；放不下的近期尾部会逐字留在表层。已配置的摘要器在该元数据未描述的窗口上作答，未公布窗口的路由则无从推导，因此两者都会重放整个可压缩头部；若 `maxTokens` 本身就耗尽窗口，则会带上具体数值发出警告。只要 `surface.replaceGeneration` 前进，就允许重试，包括剪枝在后续摘要工作抛出异常前已落地的情况。如果没有替换、目标特定上限已耗尽、已取消，或遇到未知／非规范错误，则保留原始提供方失败。
...
| `maxTokens` | 否（默认 `8192`） | 摘要调用的提供方生成上限；可包含推理 token，并会从溢出重放预算中预留出来。 |
...
- **溢出重放预算只覆盖对话自身公布的窗口**：已配置的独立摘要器，或未公布 `contextWindow` 的路由，都会重放整个可压缩头部，因此其摘要调用仍可能超出本包无法看到的窗口。
